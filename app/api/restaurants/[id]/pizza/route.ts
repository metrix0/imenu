...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (typeof body.enabled !== "boolean" || typeof body.same_category_only !== "boolean" || !["highest", "average"].includes(body.pricing_rule) || !Number.isInteger(body.max_flavors) || body.max_flavors < 2 || body.max_flavors > MAX_PIZZA_FLAVORS || !Array.isArray(body.category_ids) || body.category_ids.some((v: unknown) => typeof v !== "string" || !/^[0-9a-f-]{36}$/i.test(v))) return NextResponse.json({ error: "Configuração de pizza inválida." }, { status: 400 });
