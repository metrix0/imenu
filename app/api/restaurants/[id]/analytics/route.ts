...
type DailyRevenueRow = { date: string | Date; revenue_cents: number | string | null; orders: number | string };
...
type ItemRow = { name: string; quantity: number | string; revenue_cents: number | string | null };
type CategoryRow = { name: string; orders: number | string; quantity: number | string };
type CategoryPairRow = { first_category: string; second_category: string; orders: number | string };
...
async function getAuthenticatedUserId(request: Request): Promise<{ userId: string; email: string | null } | { response: NextResponse }> {
    const accessToken = getBearerToken(request);
    if (!accessToken) return { response: NextResponse.json({ error: "Não autenticado." }, { status: 401 }) };
    ...
    if (error || !user) return { response: NextResponse.json({ error: "Sessão inválida ou expirada." }, { status: 401 }) };
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (!id || !isDate(from) || !isDate(to)) {
            return NextResponse.json({ error: "Restaurante e período são obrigatórios." }, { status: 400 });
        }
        ...
        if (!restaurant || (restaurant.user_id !== authentication.userId && authentication.email !== ALLOWED_DEV_EMAIL)) {
            return NextResponse.json({ error: "Acesso negado ao restaurante." }, { status: 403 });
        }
        ...
        const [
            ...
            query<UnitsRow>(`
                ...
                WHERE o.restaurant_id = $1 AND o.created_at >= $2 AND o.created_at < $3 AND o.status = 'done'
            `, rangeParams),
            query<DailyRevenueRow>(`
                SELECT (created_at AT TIME ZONE $4)::date AS date, COALESCE(SUM(total_cents), 0) AS revenue_cents, COUNT(*)::int AS orders
                FROM orders
                WHERE restaurant_id = $1 AND created_at >= $2 AND created_at < $3 AND status = 'done'
                GROUP BY date ORDER BY date ASC
            `, [...rangeParams, TIME_ZONE]),
            query<DailyOrdersRow>(`
                SELECT (created_at AT TIME ZONE $4)::date AS date, COUNT(*)::int AS orders
                FROM orders
            ...
            query<DistributionRow>(`
                SELECT CASE
                    ...
                    ELSE 'unknown'
                END AS key, COUNT(*)::int AS orders
                FROM orders
                WHERE restaurant_id = $1 AND created_at >= $2 AND created_at < $3 AND status = 'done'
                GROUP BY key ORDER BY orders DESC
            `, rangeParams),
            query<HourRow>(`
                SELECT EXTRACT(HOUR FROM created_at AT TIME ZONE $4)::int AS hour, COUNT(*)::int AS orders
                FROM orders
                WHERE restaurant_id = $1 AND created_at >= $2 AND created_at < $3 AND status = 'done'
                GROUP BY hour ORDER BY hour ASC
            `, [...rangeParams, TIME_ZONE]),
            query<ItemRow>(`
                SELECT oi.name, SUM(oi.quantity)::int AS quantity, COALESCE(SUM(oi.total_cents), 0) AS revenue_cents
                FROM order_items oi INNER JOIN orders o ON o.id = oi.order_id
                WHERE o.restaurant_id = $1 AND o.created_at >= $2 AND o.created_at < $3 AND o.status = 'done'
            ...
            `, rangeParams),
            query<CategoryRow>(`
                SELECT c.name, COUNT(DISTINCT o.id)::int AS orders, SUM(oi.quantity)::int AS quantity
                FROM orders o
                INNER JOIN order_items oi ON oi.order_id = o.id
                INNER JOIN items i ON i.id = oi.item_id
                INNER JOIN categories c ON c.id = i.category_id
                WHERE o.restaurant_id = $1 AND o.created_at >= $2 AND o.created_at < $3 AND o.status = 'done'
                GROUP BY c.id, c.name ORDER BY orders DESC, quantity DESC, c.name ASC
            `, rangeParams),
            query<CategoryPairRow>(`
                WITH order_categories AS (
                    SELECT DISTINCT o.id AS order_id, c.id AS category_id, c.name
                    FROM orders o
                    INNER JOIN order_items oi ON oi.order_id = o.id
                    INNER JOIN items i ON i.id = oi.item_id
                    INNER JOIN categories c ON c.id = i.category_id
                ...
                SELECT first_category.name AS first_category, second_category.name AS second_category, COUNT(*)::int AS orders
                ...
                GROUP BY first_category.category_id, first_category.name, second_category.category_id, second_category.name
                ORDER BY orders DESC, first_category.name ASC, second_category.name ASC LIMIT 12
            `, rangeParams),
        ...
        const knownFulfillmentOrders = numberValue(summaryRow?.known_fulfillment_orders);
        ...
        const distribution = (rows: DistributionRow[]) => {
            const total = rows.reduce((sum, row) => sum + numberValue(row.orders), 0);
            return rows.map((row) => ({
                key: row.key || "unknown",
                orders: numberValue(row.orders),
                percentage: total > 0 ? Number(((numberValue(row.orders) / total) * 100).toFixed(1)) : 0,
            }));
        };
        ...
        return NextResponse.json({
            summary: {
                ...
                createdOrders,
                completedOrders,
                averageTicketCents: numberValue(summaryRow?.average_ticket_cents),
                deliveryRate: knownFulfillmentOrders > 0 ? Number(((deliveryOrders / knownFulfillmentOrders) * 100).toFixed(1)) : 0,
                averageDeliveryFeeCents: numberValue(summaryRow?.average_delivery_fee_cents),
                cancellationRate: createdOrders > 0 ? Number(((canceledOrders / createdOrders) * 100).toFixed(1)) : 0,
                averageItemsPerOrder: completedOrders > 0 ? Number((units / completedOrders).toFixed(1)) : 0,
                couponRate: completedOrders > 0 ? Number(((couponOrders / completedOrders) * 100).toFixed(1)) : 0,
            },
            revenueSeries: revenueSeriesResult.rows.map((row) => ({ date: dateValue(row.date), revenueCents: numberValue(row.revenue_cents), orders: numberValue(row.orders) })),
            orderSeries: orderSeriesResult.rows.map((row) => ({ date: dateValue(row.date), orders: numberValue(row.orders) })),
            paymentTypes: distribution(paymentResult.rows),
            fulfillment: distribution(fulfillmentResult.rows),
            hourlyOrders: hourlyResult.rows.map((row) => ({ hour: numberValue(row.hour), orders: numberValue(row.orders) })),
            items: itemsResult.rows.map((row) => ({ name: row.name, quantity: numberValue(row.quantity), revenueCents: numberValue(row.revenue_cents) })),
            categories: categoriesResult.rows.map((row) => ({ name: row.name, orders: numberValue(row.orders), quantity: numberValue(row.quantity) })),
            categoryPairs: categoryPairsResult.rows.map((row) => ({
                firstCategory: row.first_category,
                secondCategory: row.second_category,
                orders: numberValue(row.orders),
                rate: completedOrders > 0 ? Number(((numberValue(row.orders) / completedOrders) * 100).toFixed(1)) : 0,
            })),
            consumer: { postHogAvailable: consumerSeries.available, series: consumerSeries.points },
