    ...
    return <DraggableModal open={openModal} onClose={closeWithAnimation} height={1} handle>
        <div className="sticky top-0 z-10 w-full bg-white pb-3 pt-5 md:px-6 md:pt-6">
            ...
            <div className="flex items-center gap-3">
                ...
                <button className="shrink-0 cursor-pointer text-brand transition-colors hover:text-brand/80" onClick={closeWithAnimation}>Cancelar</button>
            </div>
        ...
        {!allItems.length && <p className="px-4 py-16 text-center text-gray-500 md:px-6">{debouncedSearch ? `Nenhum item encontrado para “${debouncedSearch}”.` : "Nenhum item disponível."}</p>}
        <div className="mt-3 pb-8 md:px-6">
            ...
            {hasMore && <div ref={setSentinel} className="py-4 text-center"><button className="cursor-pointer text-brand transition-colors hover:text-brand/80" onClick={() => setVisibleCount(n => n + PAGE_SIZE)}>Carregar mais</button></div>}
        </div>
    </DraggableModal>;
}
