...
export default function CartModal({
    ...
    const closeWithAnimation = () => {
        // run slide/fade out (we keep visible=false so backdrop fades)
        setVisible(false);
    ...
    const handleTouchEnd = () => {
        ...
        if (final >= DRAG_CLOSE_THRESHOLD) {
            ...
            // also hide visible/backdrop by calling closeWithAnimation after the transform
            setTimeout(() => {
    ...
    return (
        // backdrop now uses visible for fade; unchanged markup otherwise
        <div className={`fixed inset-0 z-41 flex justify-center items-end transition-opacity duration-200 ${visible ? "opacity-100 bg-black/40" : "opacity-0 bg-black/40"}`}>
            <div
                ...
                style={{
                    transform: visible
                        ? `translateY(${translateY}px)` // overrides tailwind classes when visible
                        : `translateY(100%)`,
