...
const RESTAURANT_ID = "0997e978-7f1e-46a9-88f7-81813c519485";
    ...
    return (
        <div className="p-6">
            <Button variant="primary" onClick={() => setOpen(true)}>
                Scanear cardápio com AI
            </Button>

            <ScanModal open={open} onClose={() => setOpen(false)} restaurantId={RESTAURANT_ID} />
        </div>
    );
}
 ...
 */
function ScanModal({ open, onClose, restaurantId }: { open: boolean; onClose(): void; restaurantId: string }) {
    ...
    // existingCategories: try to fetch existing categories for this restaurant (optional)
    const [existingCategories, setExistingCategories] = useState<{ id: string; name: string }[]>([]);



    useEffect(() => {
        // try fetch existing categories if your backend exposes it
        // If you don't have such endpoint just ignore the failure
        (async () => {
            try {
                const res = await fetch(`/api/categories/list?restaurant_id=${restaurantId}`);
                if (res.ok) {
                    const json = await res.json();
                    // expecting json.categories = [{id, name}, ...] — adapt if different
                    setExistingCategories(json.categories || []);
                }
            } catch (err) {
                // ignore
            }
                        ...
                        <div className="space-y-6">
                            <div className=" rounded-lg p-4">
                                <div className={"mb-6"}>
                                    ...
                                    <p className="text-sm text-gray-500">JPG, PNG, PDF. Priorize imagens do cardápio sobre fotos.</p>
                                </div>
