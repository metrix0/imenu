...
export default function TempoETaxaPage() {
    ...
    return <div className="mx-auto max-w-3xl px-4 pb-32 pt-8 sm:px-6 [&_h2>div.relative]:ml-2"><div className="panel-essencial mb-8 text-center sm:text-left"><p className="mb-2 text-xs font-bold uppercase tracking-[0.18em] text-brand">Etapa 2/4</p><h1 className="text-3xl font-bold text-gray-900">Tempo e Taxa de Entrega</h1><p className="mt-1 text-gray-500">Defina as regras de entrega para o seu restaurante.</p></div><PanelAppearance><DeliveryRules ref={rulesRef} restaurantId={restaurantId} isNew /></PanelAppearance><div className="fixed inset-x-0 bottom-0 z-40 border-t border-gray-200 bg-white p-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]"><div className="mx-auto flex max-w-4xl items-center justify-between"><button onClick={() => router.back()} className="cursor-pointer font-medium text-brand">Voltar</button><Button onClick={save} loading={saving} className="px-8">Salvar e Continuar</Button></div></div></div>;
}
