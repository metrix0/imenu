...
export default function DisponibilidadePage() {
    ...
    return <div className="flex min-h-screen flex-col bg-white"><div className="mx-auto w-full max-w-6xl flex-1 overflow-x-auto px-4 pb-32 pt-8 sm:px-6"><div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end"><div className="panel-essencial text-center sm:text-left"><p className="mb-2 text-xs font-bold uppercase tracking-[0.18em] text-brand">Etapa 2/4</p><h1 className="text-3xl font-bold text-gray-900">Horário de Funcionamento</h1><p className="mt-1 text-gray-500">Arraste os blocos para mover ou redimensionar os horários.</p></div><Button type="button" variant="secondary" onClick={recommended}>Usar horários recomendados</Button></div><WeeklyScheduleClick value={availability} onChange={setAvailability} /></div><div className="fixed inset-x-0 bottom-0 z-40 border-t border-gray-200 bg-white p-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]"><div className="mx-auto flex max-w-4xl items-center justify-between"><button onClick={() => router.back()} className="cursor-pointer font-medium text-brand">Voltar</button><Button onClick={save} loading={saving} className="px-8">Salvar e Continuar</Button></div></div></div>;
}
