...
export default function CriarCardapioPage() {
    ...
    const [profileRestaurant,setProfileRestaurant]=useState<any>(null); const [name,setName]=useState(""); const [profileStatus,setProfileStatus]=useState<SaveState>("saved"); const [methods,setMethods]=useState<string[]>(DEFAULT_ALLOWED_PAYMENT_METHODS); const [paymentInfo,setPaymentInfo]=useState("");
    ...
    const finish=async()=>{if(missingPixPayoutKey)return setToast({message:"Preencha sua chave PIX para repasses",type:"error"});if(!name.trim())return setToast({message:"Informe o nome do restaurante.",type:"error"});if(profileStatus==="saving")return setToast({message:"Aguarde os dados da loja terminarem de salvar.",type:"error"});if(profileStatus==="error")return setToast({message:"Corrija o erro ao salvar os dados da loja antes de continuar.",type:"error"});const responsiblePhoneDigits=responsiblePhone.replace(/\D/g,"");if(needsResponsiblePhone&&responsiblePhoneDigits.length!==11)return setToast({message:"Informe o celular do responsável.",type:"error"});setSaving(true);try{if(needsResponsiblePhone){const {error:phoneError}=await supabase.auth.updateUser({data:{phone:responsiblePhoneDigits}});if(phoneError)throw phoneError;}await autoSave({allowed_payment_methods:methods,first_time:false,creation_step:4});clear();router.replace("/painel");}catch(e){setToast({message:(e as Error).message,type:"error"});setSaving(false);}};
    ...
    return <main className="flex min-h-screen flex-col items-center bg-white px-4 pb-32 pt-4 sm:px-6"><div className="mt-4 w-full max-w-4xl"><div className="mb-8"><p className="mb-2 text-xs font-bold uppercase tracking-[0.18em] text-brand">Etapa 4/4</p><h1 className="text-3xl font-bold">Defina sua Loja</h1><p className="mt-1 text-gray-500">Adicione sua marca e seus primeiros produtos.</p></div>
        <PanelAppearance><div className="mb-8 space-y-8"><StoreProfileManager restaurant={profileRestaurant} hideCustomDomainButton onNameChange={setName} onPaymentInfoChange={setPaymentInfo} onSaveStatusChange={setProfileStatus}/><AllowedPaymentMethods value={methods} onChange={(next)=>{setMethods(next);void autoSave({allowed_payment_methods:next});}}/></div></PanelAppearance>
        ...
        <div className="fixed inset-x-0 bottom-0 z-40 border-t border-gray-200 bg-white p-4"><div className="mx-auto flex max-w-4xl items-center justify-between"><button onClick={()=>router.back()} className="cursor-pointer font-medium text-brand">Voltar</button><Tooltip text={missingPixPayoutKey ? "Preencha sua chave PIX para repasses" : !name.trim() ? "Você precisa completar os dados primeiro" : profileStatus==="saving" ? "Aguarde os dados terminarem de salvar" : ""}><Button onClick={finish} loading={saving} disabled={profileStatus==="saving"||missingPixPayoutKey} className={`px-8 ${!name.trim()?"!bg-brand/55 hover:!bg-brand/55":""}`}>Salvar e Continuar</Button></Tooltip></div></div>
