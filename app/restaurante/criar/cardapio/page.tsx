...
export default function CriarCardapioPage() {
    ...
    const [profileRestaurant,setProfileRestaurant]=useState<any>(null); const [name,setName]=useState(""); const [profileStatus,setProfileStatus]=useState<SaveState>("saved"); const [methods,setMethods]=useState<string[]>(DEFAULT_ALLOWED_PAYMENT_METHODS);
    ...
    const load = async () => {
        ...
        try {
            ...
            setProfileRestaurant(restaurant); setName(restaurant.name||""); setMethods(Array.isArray(restaurant.allowed_payment_methods)&&restaurant.allowed_payment_methods.length?restaurant.allowed_payment_methods:DEFAULT_ALLOWED_PAYMENT_METHODS);
    ...
    const finish=async()=>{if(!name.trim())return setToast({message:"Informe o nome do restaurante.",type:"error"});if(profileStatus==="saving")return setToast({message:"Aguarde os dados da loja terminarem de salvar.",type:"error"});if(profileStatus==="error")return setToast({message:"Corrija o erro ao salvar os dados da loja antes de continuar.",type:"error"});const responsiblePhoneDigits=responsiblePhone.replace(/\D/g,"");if(needsResponsiblePhone&&responsiblePhoneDigits.length!==11)return setToast({message:"Informe o celular do responsável.",type:"error"});setSaving(true);try{if(needsResponsiblePhone){const {error:phoneError}=await supabase.auth.updateUser({data:{phone:responsiblePhoneDigits}});if(phoneError)throw phoneError;}await autoSave({allowed_payment_methods:methods,first_time:false,creation_step:4});clear();router.replace("/painel");}catch(e){setToast({message:(e as Error).message,type:"error"});setSaving(false);}};
