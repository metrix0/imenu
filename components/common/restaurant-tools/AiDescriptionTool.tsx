...
export default function AiDescriptionTool() {
    ...
    return (
        <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
            <ToolPanel title="Dados verdadeiros do produto">
            ...
            <ToolPanel title="Descrições geradas" description="Escolha e adapte a versão que mais combina com o seu cardápio.">
                {result ? (
                    <div className="space-y-4">
                        {result.descriptions.map((description, index) => (
                            <article key={`${description}-${index}`} className="rounded-xl border border-gray-200 bg-gray-50 p-4">
                                <div className="flex items-start justify-between gap-3">
                                    ...
                                    <button type="button" className="cursor-pointer text-xs font-semibold text-brand hover:underline" onClick={() => void copy(`option-${index}`, description)}>
                        ...
                        <article className="rounded-xl border border-brand/25 bg-brand/5 p-4">
                            <div className="flex items-start justify-between gap-3">
                                ...
                                <button type="button" className="cursor-pointer text-xs font-semibold text-brand hover:underline" onClick={() => void copy("short", result.shortDescription)}>
