...
const DEFAULT_DELIVERY_RULES: DeliveryRule[] = [
...
const normalizeDeliveryRules = (value: unknown): DeliveryRule[] => {
  if (!Array.isArray(value)) {
    return DEFAULT_DELIVERY_RULES.map((rule) => ({ ...rule }));
  }
  ...
  const normalized = value
    .map((rule) => {
      ...
      const radiusKm = Number(typedRule.radius_km);
      const timeMinutes = Number(typedRule.time_minutes);
      const feeCents = Number(typedRule.fee_cents);
    ...
    .filter((rule): rule is DeliveryRule => rule !== null)
    .sort((first, second) => first.radius_km - second.radius_km);

  return normalized.length > 0
    ? normalized
    : DEFAULT_DELIVERY_RULES.map((rule) => ({ ...rule }));
};
...
export default async function Page({
  ...
  const restaurant: Restaurant & { address: any } = {
    ...
    delivery_fee_json: normalizeDeliveryRules(
      restaurantData.delivery_fee_json,
    ),
